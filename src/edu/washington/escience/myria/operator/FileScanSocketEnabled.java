...
import edu.washington.escience.myria.io.FileSourceSocketEnabled;
 ...
 *
 */
public final class FileScanSocketEnabled extends LeafOperator {
  private static java.net.Socket _inputSocket;
  ...
  public synchronized static <T> T getStream(Object instance, Class<T> ioClass) {
      try {
        if(ioClass.equals(java.io.InputStream.class)) {
          if(_inputSocket == null)
	      _inputSocket = new java.net.Socket("localhost", 9999);
	  //_inputSocket = new java.net.ServerSocket(9999);
          return (T)_inputSocket.getInputStream();
        }
        else {
          if(_outputSocket == null)
	      _outputSocket = new java.net.ServerSocket(9999);
	  //_outputSocket = new java.net.Socket("localhost", 9999);
  ...
  private final DataSourceSocketEnabled source;
   ...
   */
  private static final org.slf4j.Logger LOGGER = org.slf4j.LoggerFactory.getLogger(FileScanSocketEnabled.class);

  /**
   * Construct a new FileScanSocketEnabled object to read from the specified file. This file is assumed to be comma-separated and
   ...
   */
  public FileScanSocketEnabled(final String filename, final Schema schema) {
   ...
   * @param schema the Schema of the relation contained in the file.
   */
  public FileScanSocketEnabled(final DataSourceSocketEnabled source, final Schema schema) {
   ...
   */
  public FileScanSocketEnabled(final String filename, final Schema schema, final Character delimiter) {
    this(new FileSourceSocketEnabled(filename), schema, delimiter, null, null, null);
  }
   ...
   * @param delimiter An optional override file delimiter.
   */
  public FileScanSocketEnabled(final DataSourceSocketEnabled source, final Schema schema, final Character delimiter) {
   ...
   */
  public FileScanSocketEnabled(final String filename, final Schema schema, @Nullable final Character delimiter,
      @Nullable final Character quote, @Nullable final Character escape, @Nullable final Integer numberOfSkippedLines) {
    this(new FileSourceSocketEnabled(filename), schema, delimiter, quote, escape, numberOfSkippedLines);
  }
   ...
   */
  public FileScanSocketEnabled(final DataSourceSocketEnabled source, final Schema schema, @Nullable final Character delimiter,
      @Nullable final Character quote, @Nullable final Character escape, @Nullable final Integer numberOfSkippedLines) {
