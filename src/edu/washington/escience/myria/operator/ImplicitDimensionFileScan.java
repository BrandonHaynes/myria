 ...
 * 
 */
public final class ImplicitDimensionFileScan extends LeafOperator {
   ...
   */
  public ImplicitDimensionFileScan(final String filename, final Schema schema, final List<Integer> dimensions) {
    this(filename, schema, null, null, null, null, dimensions);
  }
   ...
   * @param schema the Schema of the relation contained in the file.
   */
  public ImplicitDimensionFileScan(final DataSource source, final Schema schema, final List<Integer> dimensions) {
    this(source, schema, null, null, null, null, dimensions);
  }
   ...
   */
  public ImplicitDimensionFileScan(final String filename, final Schema schema, final Character delimiter, final List<Integer> dimensions) {
    this(new FileSource(filename), schema, delimiter, null, null, null, dimensions);
  }
   ...
   * @param delimiter An optional override file delimiter.
   */
  public ImplicitDimensionFileScan(final DataSource source, final Schema schema, final Character delimiter, final List<Integer> dimensions) {
    this(source, schema, delimiter, null, null, null, dimensions);
  }
   ...
   */
  public ImplicitDimensionFileScan(final String filename, final Schema schema, @Nullable final Character delimiter,
    ...
    this(new FileSource(filename), schema, delimiter, quote, escape, numberOfSkippedLines, dimensions);
  }
   ...
   */
  public ImplicitDimensionFileScan(final DataSource source, final Schema schema, @Nullable final Character delimiter,
      @Nullable final Character quote, @Nullable final Character escape, @Nullable final Integer numberOfSkippedLines,
      final List<Integer> dimensions) {
  ...
  protected TupleBatch fetchNextReady() throws DbException, IOException {
    ...
    while ((buffer.numTuples() < TupleBatch.BATCH_SIZE)) {
      ...
      if (record.size() + dimensions.size() != schema.numColumns()) {
        throw new DbException("Error parsing row " + lineNumber + ": Found " + record.size()
            + " column(s) but expected " + schema.numColumns() + " column(s).");
      }
      for (int column = dimensions.size(); column < schema.numColumns(); ++column) {
        String cell = record.get(column - dimensions.size());
        try {
