...
/**
 * A {@link TupleWriterSocketEnabled} is an object that can serialize a {@link ReadableTable}. The assumption is that most classes
 * that implement {@link TupleWriterSocketEnabled} will write the data to an {@link java.io.OutputStream}, e.g.
 * 
 ...
 * 
 */
public interface TupleWriterSocketEnabled {

  /**
   * Inform the {@link TupleWriterSocketEnabled} of the column headers. In the standard case (CSV output, see {@link CsvTupleWriterSocketEnabled}
   * ), they are written out directly. In some cases, they may be cached and output with the data (see
   * {@link JsonTupleWriterSocketEnabled}).
   * 
  ...
  /**
   * Provide a {@link TupleWriterSocketEnabled} with tuples to serialize.
   * 
  ...
  /**
   * Inform the {@link TupleWriterSocketEnabled} that no more tuples will come. It may close and flush an
  ...
  /**
   * Called when the query filling in the {@link TupleWriterSocketEnabled} has experienced an error. The {@link TupleWriterSocketEnabled} should
